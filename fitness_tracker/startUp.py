...
if ans == 'y':
    ...
    if ans == 'y':
        [protein_goal, carbs_goal, fat_goal,calories_goal] = dietCalc.cutMacros(weight,.2)    #We'll just assume a 20% body fat to start
    else:
        [protein_goal, carbs_goal, fat_goal,calories_goal] = dietCalc.maintMacros(weight)

print("Your macros to start are:")
print('Calories: {:.0f} \t Protein: {:.0f}\t Carbs: {:.0f}\t Fat: {:.0f}'.format(calories_goal, protein_goal, carbs_goal, fat_goal))

ans = input("OK cool. Now that that is out of the way, do you want to take body measurements? (y/n):\n")
if ans == 'y':
